...
async function processAuthUser(user) {
  ...
  const _le = document.getElementById('login-email'); if (_le) _le.disabled = false;
  const _lp = document.getElementById('login-pw'); if (_lp) _lp.disabled = false;
  const _lr = document.getElementById('login-error'); if (_lr) _lr.textContent = '';
}
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
      try {
        ...
        const _le = document.getElementById('login-email'); if (_le) _le.disabled = false;
        const _lp = document.getElementById('login-pw'); if (_lp) _lp.disabled = false;
        const _lr2 = document.getElementById('login-error'); if (_lr2) _lr2.textContent = 'Villa við innskráningu. Reyndu aftur.';
        openParentLoginPopup();
      }
      return;
    }
